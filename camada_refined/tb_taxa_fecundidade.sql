...
CREATE TABLE dados_refined.tb_taxa_fecundidade AS (
WITH refinando_pt1 AS ( 
	SELECT *
	FROM dados_trusted.tb_trusted_fecundidade
	WHERE cls_nome = 'Ano'
),
refinando_pt2 AS (
	SELECT *
...
SELECT DISTINCT 
	...
	, CAST(pt2.cls_id AS INTEGER) as cls_id_grp_mae
	, CAST(pt2.cls_categoria AS TEXT) as cls_categoria_grp_mae
...
ORDER BY cls_categora_ano_base
) 