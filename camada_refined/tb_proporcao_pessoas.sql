...
CREATE TABLE dados_refined.tb_proporcao_pessoas AS (
WITH refinando_pt1 AS ( 
	SELECT *
	FROM dados_trusted.tb_trusted_proporcao_pessoas
	WHERE cls_nome = 'Ano'
),
refinando_pt2 AS (
	SELECT *
...
SELECT CAST(pt1.id AS INTEGER) AS id
	...
	, CAST(pt2.cls_id AS INTEGER) as cls_id_grp_idade
	, CAST(pt2.cls_categoria AS TEXT) as cls_categoria_grp_idade
